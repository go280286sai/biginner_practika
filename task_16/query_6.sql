select count(carts_id), round(sum(price)) as price from public.users inner join public.carts on public.users.user_id=users_user_id inner join public."'Order'"
...
public.categories.category_id=public.products.category_id where status_name='Canceled' GROUP BY carts_id